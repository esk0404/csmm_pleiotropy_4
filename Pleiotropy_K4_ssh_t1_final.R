...
#------------------------------------------------------------------------------------------
here::i_am("Pleiotropy_K4_ssh_t1_final.R")
...
res <- symm_fit_ind_EM_R1(t_value = 1, 
...
# 10. Sort lfdrResults and compute cumulative average and map back to original SNPs
sorted_idx <- order(res$lfdrResults)
cum_avg <- cumsum(res$lfdrResults[sorted_idx]) / seq_along(sorted_idx)
cum_avg_original <- numeric(length(res$lfdrResults))
cum_avg_original[sorted_idx] <- cum_avg


pleio_4_analysis <- prepped_data$extra_dat %>%
  mutate(
    lfdrResults = res$lfdrResults,
    cum_avg = cum_avg_original)


# 11. Find replicated SNPs
sig_snps_info <- pleio_4_analysis %>%
  filter(cum_avg < 0.1) %>%
  arrange(lfdrResults) %>%
  select(Chr, BP, EA, OA, rsid, lfdrResults, cum_avg, 
         Z_Breast, Z_Lung, Z_Ovarian, Z_Prostate,
         P_Breast, P_Lung, P_Ovarian, P_Prostate
  ) 


# outputs
## results
outfile <- file.path(outputDir, "Pleiotropy_K4_ssh_t1.csv")
write.csv(sig_snps_info, outfile, row.names = FALSE)
